...
核心不依赖 React、Electron 或任何宿主布局引擎；涉及 React 的代码只在 `view-anchor/react` 适配层。
...
React 适配层，从 `view-anchor/react` 导入，返回一个挂到占位元素上的 ref 回调。
...
const ref = useViewAnchor({
  present,            // boolean
  publish,            // Publisher<Bounds>；同步 false 表示未接收
...
| `opts` / `deps` 变化 | `update` |
| 真实卸下 | 最迟在一个 microtask 内发零矩形，再 `dispose` |

**`deps`** —— `ResizeObserver` 只响应纯几何变化。会移动矩形却不改变被观察元素尺寸的状态（布局拓扑签名、路由切换、兄弟标签页 `display:none`）要放进 `deps`。数组长度需在每次渲染间保持稳定。

React 18 以 `ref(null)` 表示卸下；React 19 可以调用 ref 返回的清理函数。适配层把隐藏延后一个 microtask：React 19 开发模式的清理后立即重新挂载会取消隐藏，真实卸下则在该 microtask 内隐藏并释放。React 18/19 StrictMode 重放和 hidden→shown 重挂载都不会产生多余的隐藏状态。

## usePlacementAnchor(opts)

`usePlacementAnchor` 也从 `view-anchor/react` 导入。它把 `createPlacementAnchor` 的 `visible`、`followScroll`、`followGeometry` 和 `guardDisplayNone` 交给 React ref 生命周期管理，返回同样的 ref 回调；真实卸下时发布 `{ visible:false }` 后释放。
...
| `src/react.ts` | `view-anchor/react` 适配层：`useViewAnchor` 和 `usePlacementAnchor`。 |
...
| `src/index.ts` | `view-anchor` 核心入口；不加载 React。 |

核心运行时只依赖浏览器 API（`ResizeObserver`、`getBoundingClientRect`、`window` 的 `resize` 监听）；`view-anchor/react` 额外依赖 React。`requestAnimationFrame` 只在反向 `createSizeAdvertiser`（经 `createMeasureLoop`）和 `createPlacementAnchor` 的 opt-in `followGeometry` 哨兵里用；正向的 `createViewAnchor` 同步发布、不走 RAF。
