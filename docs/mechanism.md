...
多个锚同时开启 `followScroll` 时，一次能到达 `window` 的滚动事件会通知每个活动锚；`dedupe` 只减少发布，不减少 `getBoundingClientRect()` 读取。配合 `followGeometry` 的回归测试中，16 个锚共用一个滚动容器、仅 1 个锚移动：触发后共测量 33 次（移动的锚 3 次，其余各 2 次），只发布 1 次，随后没有待执行的帧；全部 `dispose()` 后再滚动，测量次数为 0。这是用受控帧测得的调用次数，**不是**真实浏览器的布局耗时。大量锚同时开启该选项的实际帧耗时仍需在浏览器中测量，不能仅凭发布次数推断。
